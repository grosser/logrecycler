...
describe "logrecycler" do
  ...
  it "fails with unknown arguments" do
    call("--wut", expected_exit: 2).must_include "Usage"
  end
  ...
  it "shows location when failing on bad regex in preprocess" do
    with_config "preprocess: '((((WUT'" do
      call("", expected_exit: 2).must_equal "Error in regular expression from preprocess: error parsing regexp: missing closing ): `((((WUT`"
    end
  end

  it "shows location when failing on bad regex in pattern" do
    with_config "patterns:\n- regex: '((((WUT'" do
      call("", expected_exit: 2).must_equal "Error in regular expression from patterns[0].regex: error parsing regexp: missing closing ): `((((WUT`"
    end
  end
